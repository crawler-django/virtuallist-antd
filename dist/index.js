...
// =============组件 =================== //
function VRow(props) {
    var _a = React.useContext(ScrollContext), dispatch = _a.dispatch, fixed = _a.fixed, rowHeight = _a.rowHeight;
...
function VWrapper(props) {
    ...
    // console.log(trs)
    return (React__default.createElement("tbody", __assign({}, restProps, { style: { transform: "translateY(-" + offsetStart + "px)" } }), trs));
}
function VTable(props) {
    ...
    , ["width"]);
    // const [curScrollTop, setCurScrollTop] = useState(0)
    var _p = React.useReducer(reducer, initialState), state = _p[0], dispatch = _p[1];
    ...
    return (React__default.createElement("div", { ref: wrap_tableRef, style: {
            width: width,
            position: 'relative',
            height: tableHeight,
            boxSizing: 'border-box',
        ...
        React__default.createElement(ScrollContext.Provider, { value: {
            ...
            React__default.createElement("table", __assign({}, rest, { ref: tableRef, style: __assign(__assign({}, rest_style), { width: width, position: 'relative' }) }), children))));
}
// ================导出===================
function VList() {
    // 初始化staticRowHeight
